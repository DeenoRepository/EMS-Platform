...
export async function POST(
  ...
  const rateLimitError = await enforceRateLimit(req, { limit: 30, windowMs: 60 * 1000, prefix: 'eps-photo-upload' });
...
export async function DELETE(
  ...
  const rateLimitError = await enforceRateLimit(req, { limit: 30, windowMs: 60 * 1000, prefix: 'eps-photo-delete' });
  if (rateLimitError) return rateLimitError;
