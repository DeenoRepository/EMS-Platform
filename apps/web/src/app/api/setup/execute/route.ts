...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (fs.existsSync(installedFilePath) || fs.existsSync(rootInstalledFilePath)) {
      return NextResponse.json(
        { success: false, error: 'Система уже установлена и настроена. Повторная установка заблокирована.' },
        { status: 403 }
      );
    }
    ...
    const envLines = [
      ...
      `STORAGE_TYPE="local"`,
      `STORAGE_LOCAL_DIR="${storageConfig?.dir || './uploads'}"`,
      ``,
