...
export async function POST(
  ...
  const rateLimitError = await enforceRateLimit(req, { limit: 60, windowMs: 60 * 1000, prefix: 'wms-transfer-reject' });
  ...
  try {
    ...
    if (notifyUserId && notifyUserId !== user.userId) {
      await prisma.notification.create({
        ...
        logger.warn('Не удалось отправить уведомление об отклонении перемещения', {
