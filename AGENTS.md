...
> **Версия правил:** 2.1 (2026-09-06: безопасная модуляризация и сохранность данных)
...
Прочитать [правила безопасных изменений](.agents/rules/safe_changes.md) перед изменением кода, схемы, установщиков или конфигурации. Для вывода прототипов и модуляризации следовать [плану передачи](docs/SRM_MRO_RETIREMENT_MODULARIZATION_PLAN.md). Эти ограничения уточняют старые примеры и приоритеты в правилах проекта; инструкции более высокого уровня сохраняют приоритет.

**SRM/MRO — прототипы, запланированные к выводу из активного приложения, а не законченные модули.** Не развивать и не рефакторить их ради старого аудита. Сам план не разрешает автоматически удалять код, данные или выполнять деплой: работать только в рамках текущей задачи пользователя.

Первый функциональный релиз перехода не меняет физическую схему БД. Не удалять модели, историю, остатки, роли и паспортные поля ТО. Версия production и фактическая схема должны быть проверены отдельно от локального checkout. Текстовые правила не заменяют еще не внедренные архитектурные проверки CI.

## 🚨 Обязательные правила разработки

### 1. Фиксация изменений (Git Commit)
* **После каждого проверенного этапа с изменениями агент ОБЯЗАН выполнять отдельный Git-коммит только своих файлов.** Для консультации без изменений пустой коммит не нужен. Сначала проверить рабочее дерево и индекс; не включать чужие изменения и не отменять их. Перед реализацией из detached HEAD создать именованную рабочую ветку от согласованной базы. Push, deploy и переписывание истории требуют отдельного разрешения.
...
* **ХАРДКОД UI КАТЕГОРИЧЕСКИ ЗАПРЕЩЕН**:
  ...
  * ✅ Новый типовой UI-элемент оформлять в существующей общей библиотеке [UI](apps/web/src/components/ui/) и применять в затронутых активных разделах. Не изменять прототипы SRM/MRO ради сквозной унификации. При согласованном выделении UI в workspace-пакет переносить существующие компоненты без дублирования; доменный пакет не должен импортировать web-приложение.
...
Перед профильной задачей выбрать один наиболее специфичный применимый скилл из доступных описаний. Список ниже — варианты, не требование загрузить все сразу. Проверять наличие инструкций, инструментов и API; отсутствующий файл или инструмент сообщать как ограничение, а не выдумывать результат:
...
| Тема | Файл |
|---|---|
| Безопасные изменения, модульные границы, БД и деплой | [`.agents/rules/safe_changes.md`](.agents/rules/safe_changes.md) |
