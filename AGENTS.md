...
> **Версия правил:** 3.1 (2026-09-02 — обязательное тестирование нового кода)
...
### 4. Обязательное тестирование нового кода
* **Весь новый код с поведением** (бизнес-логика, API-роуты, доменные
  сервисы, чистые функции, компоненты с ветвлением) **ОБЯЗАН
  сопровождаться исполняемым тестом в том же коммите.** Тест — часть
  готовности функциональности, а не отдельная последующая задача.
* Исправление бага начинается с теста, воспроизводящего баг.
* ❌ Запрещены тесты, дублирующие логику локальной копией вместо импорта
  продакшн-модуля, и проверки исходного текста регуляркой вместо исполнения.
* Автор обязан убедиться, что тест **краснеет при регрессии**: зелёный тест
  сам по себе ничего не доказывает.
* Пороги покрытия — ratchet: их не понижают, чтобы «протолкнуть» изменение.
* Полные правила, оси покрытия для write-роутов и фактический стек
  раннеров: [`.agents/rules/testing.md`](.agents/rules/testing.md).
...
| Обязательное тестирование нового кода | [`.agents/rules/testing.md`](.agents/rules/testing.md) |
