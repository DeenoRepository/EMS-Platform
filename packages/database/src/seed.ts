...
async function main() {
  ...
  const customFieldsData = [
    ...
    },
    {
      sectionId: sectionElectrical.id,
      key: 'ups_required',
      name: 'Требование к наличию источника бесперебойного питания (ИБП)',
      fieldType: FieldType.SELECT,
      options: ['Требуется', 'Не требуется'],
    ...
    // 5. Механика, гидравлика и среда
    {
      ...
      name: 'Номинальное рабочее давление',
      fieldType: FieldType.NUMBER,
      unit: 'бар',
      defaultValue: '16',
      sortOrder: 1,
    },
    {
      sectionId: sectionMechanics.id,
      key: 'rotation_speed',
      name: 'Номинальная частота вращения вала',
      fieldType: FieldType.NUMBER,
      unit: 'об/мин',
      defaultValue: '1500',
      sortOrder: 2,
    },
    {
      sectionId: sectionMechanics.id,
      key: 'coolant_type',
      name: 'Рабочая среда / Смазочные материалы (ГОСТ / ISO)',
      fieldType: FieldType.TEXT,
  ...
  const equipment1 = await prisma.equipment.upsert({
    ...
    create: {
      ...
      customFields: {
        ...
        maintenance_count: 4,
        responsible_person_name: 'Петров А.В. (Ведущий инженер-механик)',
        external_system_id: '1C-OS-4401',
        power_kw: 15,
        operating_voltage: '380 В',
        nominal_current: 28,
        phase_count: 3,
        ups_required: 'Не требуется',
        operating_pressure: 16,
        rotation_speed: 1500,
        coolant_type: 'Вода техническая оборотная (ГОСТ 2874-82)',
  ...
  const equipment2 = await prisma.equipment.upsert({
    ...
    create: {
      ...
      customFields: {
        ...
        rotation_speed: 3000,
        coolant_type: 'Масло синтетическое Atlas Copco Roto Inject Fluid (ISO VG 46)',
        is_critical_path: true,
        calibration_interval: 12,
      },
      createdById: adminUser.id,
  ...
  const equipment3 = await prisma.equipment.upsert({
    ...
    create: {
      ...
      model: 'Topvex FR 06',
      location: 'Венткамера главного корпуса, поз. В-1',
      status: EquipmentStatus.UNDER_REPAIR,
      commissionDate: new Date('2020-01-10'),
      customFields: {
        okof_code: '330.28.25.12',
        okpd2_code: '28.25.12.110',
        process_classifier_code: 'ТП-ВЕНТ-01',
        equipment_group: 'Энергетическое и вспомогательное',
        equipment_type: 'Вентиляционные установки',
        actual_wear_percentage: 24,
        criticality: 'Категория B (Основное / Дублируемое)',
        clean_room_class: 'Не регламентируется',
        is_unique: false,
        is_imported: true,
        maintenance_periodicity: '1 раз в квартал (ТО-1)',
  ...
  const nom1 = await prisma.nomenclature.upsert({
    ...
    create: {
      ...
      description: 'Для подшипникового узла электродвигателя насоса Grundfos NB 50-200/219 (размеры 20×47×14 мм)',
      minStock: 5,
    },
  });
  ...
  const nom2 = await prisma.nomenclature.upsert({
    ...
    update: {},
    create: {
      name: 'Торцевое механическое уплотнение вала 25×42×10 мм NBR/Carbon/Ceramic (ГОСТ 8752-79)',
      article: 'SEAL-25-42-10',
      unit: 'шт',
      categoryId: catParts.id,
      description: 'Для герметизации гидравлической части центробежного насоса подачи охлаждающей воды',
      minStock: 2,
    },
  });
  ...
  const nom3 = await prisma.nomenclature.upsert({
    ...
    update: {},
    create: {
      name: 'Масло компрессорное синтетическое Atlas Copco Roto Inject Fluid (ISO VG 46)',
      article: 'OIL-VG-46',
      unit: 'л',
      categoryId: catOils.id,
      description: 'Смазка и охлаждение винтового блока компрессора Atlas Copco GA 45 VSD+ (канистра 20 л)',
      minStock: 20,
    },
  });
  ...
  await prisma.stockItem.upsert({
    ...
    update: { quantity: 60 },
    create: { warehouseId: mainWarehouse.id, nomenclatureId: nom3.id, quantity: 60 },
  });
