...
> Перед изменениями обязательны [правила безопасных изменений](safe_changes.md). SRM/MRO — выводимые прототипы; старые примеры не разрешают их доработку или удаление данных.
...
* **После проверенного этапа с изменениями создать отдельный коммит только своих файлов.** Проверить индекс и рабочее дерево; чужие изменения не включать и не отменять. Перед реализацией из detached HEAD создать именованную ветку от согласованной базы. Для консультации пустой коммит не нужен. Push, deploy и переписывание истории — только по отдельному разрешению.
...
Выбрать один наиболее специфичный применимый скилл по его актуальному описанию, не загружать все перечисленные сразу. Проверять наличие файлов, инструментов и фактические версии API. Отсутствующие проверки нельзя объявлять пройденными. Следующие пункты — ориентиры, а не разрешение расширять задачу.
...
* **strict-api**: проверка существования методов, импортов и API в установленной версии; не заменяет runtime-валидацию входных данных.
...
* **jira-expert**: явно согласованные задачи Jira. Не развивать прототип SRM ради старых рекомендаций; при выводе проверить отключение внешних отправителей.
...
## Архитектура проекта (историческая справка checkout)

Дерево ниже описывает размещение файлов, а не завершенные модульные границы. SRM/MRO — прототипы к выводу; EPS/WMS предстоит выделить по [плану](../../docs/SRM_MRO_RETIREMENT_MODULARIZATION_PLAN.md). Перед изменениями проверять реальную структуру. Пакеты не должны импортировать приложение, в том числе ради общей UI-библиотеки.
