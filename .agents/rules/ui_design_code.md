...
5. Применить в затронутых активных разделах EPS, WMS и администрирования. Не изменять выводимые прототипы SRM/MRO ради унификации.
6. Перед переносом UI в workspace-пакет прочитать [правила безопасных изменений](safe_changes.md). Переносить существующие контролы без дублирования; доменный пакет не импортирует web-хост. Примеры путей выше отражают текущую библиотеку, а не запрет ее согласованного выделения.
