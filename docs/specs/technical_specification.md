...
## 6. Модуль SRM 2.0 — Управление сервисными заявками, инцидентами и надежностью оборудования (Service & Reliability Management)

### 6.1 Архитектурная концепция SRM 2.0

> [!NOTE]
> В версии SRM 2.0 модуль трансформирован из пассивного Jira-дашборда в полноценный гибридный **Enterprise ServiceDesk & RAMS/RCM Аналитический Центр**:
> - **Собственный внутренний ServiceDesk**: оперативная регистрация поломок, простоев, аварийных инцидентов и вызовов ремонтных бригад;
> - **Двусторонняя интеграция с внешними системами**: Jira, Redmine, 1C:ТОиР, GitLab Issues, кастомные REST API и входящие Webhooks;
> - **Сквозной процесс надежности (Closed-loop)**: Инцидент $\rightarrow$ Автоматический перевод оборудования EPS в `UNDER_REPAIR` $\rightarrow$ Формирование аварийного наряда MRO $\rightarrow$ Резервирование/списание запчастей WMS $\rightarrow$ Расчет MTTR/MTBF/КТГ $\rightarrow$ Возврат оборудования в `ACTIVE`.

### 6.2 Функциональные требования

#### 6.2.1 Внутренний ServiceDesk и регистрация заявок
- Регистрация инцидентов с присвоением уникального ключа (формат `INC-YYYY-XXXX`).
- Выбор единицы оборудования с живым автокомплитом по названию или инвентарному номеру.
- Автоматический расчет SLA Deadline в зависимости от критичности (CRITICAL: 4ч, HIGH: 12ч, MEDIUM: 24ч, LOW: 72ч).
- Автоматический перевод актива в статус `UNDER_REPAIR` при фиксации критических инцидентов.
- Отправка системных push-уведомлений дежурной службе ТОиР.

#### 6.2.2 RAMS и RCM Аналитика надежности оборудования
- **Коэффициент технической готовности (КТГ / Availability %)**:
  $$\text{КТГ} = \frac{\text{Время работы}}{\text{Время работы} + \text{Время простоя}} \times 100\%$$
- **MTTR (Mean Time To Repair)**: средняя продолжительность локализации и устранения отказа.
- **MTBF (Mean Time Between Failures)**: средняя наработка между отказами.
- **Диаграмма Парето (Анализ 80/20)**: выявление 20% корневых дефектов, формирующих 80% суммарного простоя технологических линий.
- **ТОП-10 проблемного оборудования**: ранжирование агрегатов по частоте отказов и часам простоя для планирования углубленного ППР и модернизации.

#### 6.2.3 Учет гарантийных случаев и рекламаций (Warranty & Contractor Claims)
- Фиксация признака заводской гарантии при регистрации дефекта.
- Учет авторизованных сервисных центров и подрядчиков по гарантии.
- Контроль соблюдения гарантийных SLA поставщиков и процента возмещения расходов.

#### 6.2.4 Интеграционный шлюз и Конструктор сопоставления полей (Field Mapping Builder)
- Поддержка готовых адаптеров: **Jira Cloud/Server**, **Redmine**, **1С:ТОиР / 1С:ERP**, **GitLab**, **Generic REST / Webhooks**.
- Интерактивный UI конструктора маппинга (JSON dot-нотация, трансформаторы типов, кастомные поля).
- Встроенный интерактивный тестер (*Live Tester*) сопоставления JSON без перезагрузки сервера.
- Прием событий изменений статусов через индивидуальные URL вебхуков (`/api/srm/webhooks/:integrationId`).

### 6.3 Интеграционный поток и жизненный цикл инцидента

```mermaid
sequenceDiagram
    autonumber
    actor Tech as Инженер / Оператор
    participant SRM as Модуль SRM 2.0
    participant EPS as Модуль EPS (Паспортизация)
    participant MRO as Модуль MRO (ТОиР)
    participant WMS as Модуль WMS (Склад)
    participant Ext as Внешние API (Jira / 1C)

    Tech->>SRM: Регистрация инцидента / поломки
    SRM->>EPS: Смена статуса актива на "UNDER_REPAIR"
    SRM-->>Ext: Синхронизация внешнего тикета (опционально)
    Tech->>SRM: Клик "Сформировать заказ-наряд ТОиР"
    SRM->>MRO: Создание аварийного наряда (с привязкой к SRM)
    MRO->>WMS: Подбор и списание необходимых ЗИП
    Tech->>SRM: Завершение ремонта (фиксация простоя и резолюции)
    SRM->>EPS: Возврат статуса актива в "ACTIVE"
    SRM->>SRM: Пересчет MTTR, MTBF, КТГ и Парето-аналитики
```
