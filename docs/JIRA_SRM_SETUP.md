...
## 5. Защита входящих SRM webhooks

Активная SRM-интеграция обязана иметь секрет входящего webhook. Секрет передаётся в одном из заголовков `x-webhook-secret`, `x-gitlab-token`, `x-api-key`, `Authorization: Bearer ...` или в query-параметре `token` / `secret`.

Если интеграция не может подписывать webhook, unsigned-режим разрешается только явным флагом `allowUnsignedWebhooks=true` в `authConfig`. Это снижает защиту от поддельных входящих событий и должно использоваться только в доверенной внутренней сети.

Секреты маскируются в API-ответах и не должны попадать в документацию, логи или клиентский код.

---

## 6. Доступные REST API эндпоинты Jira Mock
