...
exports.parseJiraXmlFile = parseJiraXmlFile;
 ...
 */
function parseJiraXmlFile(filePath, baseUrl = 'http://localhost:8080') {
    ...
    while ((itemMatch = itemRegex.exec(xmlContent)) !== null) {
        ...
        const getTagAttr = (tagName, attrName) => {
        ...
        return new Promise((resolve) => {
            this.server = http.createServer((req, res) => {
                ...
                if (pathname === '/rest/api/2/search' || pathname === '/rest/api/3/search') {
                    let jql = parsedUrl.query.jql || '';
                    let startAt = parseInt(parsedUrl.query.startAt || '0', 10);
                    let maxResults = parseInt(parsedUrl.query.maxResults || '50', 10);
                    if (req.method === 'POST') {
                        let bodyStr = '';
...
exports.JiraMockServer = JiraMockServer;
// Standalone runner
